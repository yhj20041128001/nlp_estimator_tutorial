...
class StateStore:
    ...
    用 JSON 文件持久化两类状态：
      ...
      - last_active:   {node_key: {user@host: iso_timestamp}}
    """
    ...
    def load(self) -> dict:
        ...
        try:
            ...
            logging.getLogger("account_monitor").warning(
                "状态文件损坏或读取失败，重新初始化: %s", self._path
            )
            return {}

    def save(self):
        """原子写入：先写临时文件再重命名，防止中途崩溃损坏文件。"""
        ...
        try:
            ...
            logging.getLogger("account_monitor").error(
                "状态文件写入失败: %s", e
            )
...
def init_storage_table(config, logger):
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute(_CREATE_TABLE_SQL_TPL.format(table=table))
        conn.commit()
        cursor.close()
        logger.info("存储库初始化完成，表: %s.%s", st_cfg["database"], table)
    except Exception as e:
...
def main():
    ...
    state_file_name = monitor_cfg.get("state_file", "account_monitor_state.json")
    state = StateStore(BASE_DIR / state_file_name)
    ...
    logger.info("  状态文件: %s", BASE_DIR / state_file_name)
